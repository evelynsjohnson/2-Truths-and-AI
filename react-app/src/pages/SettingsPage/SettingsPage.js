...
export default function SettingsPage() {
  ...
  const handleCustomColorChange = (e) => {
    ...
    // Derive a slightly darker secondary and a subtle hover shade from the chosen primary
    const derivedSecondary = darkenHex(color, 18); // ~18% darker
    const derivedSecondaryHover = darkenHex(color, 8); // ~8% darker than primary for hover states

    updateSettings({ customPrimary: color, customSecondary: derivedSecondary, customSecondaryHover: derivedSecondaryHover });

    if (settings.theme === 'theme-custom') {
      document.documentElement.style.setProperty('--primary', color);
      document.documentElement.style.setProperty('--secondary', derivedSecondary);
      document.documentElement.style.setProperty('--secondary-hover', derivedSecondaryHover);
    }
  };
